...
/**
 * use a custom stdout/stderr maxBuffer
 * @todo to avoid using MAX_BUFFER change
 * child_process.exec call for child_process.spawn
 */
...
const taskListHeader = 'Image Name                     PID Session Name        Session#    Mem Usage Status          User Name                                              CPU Time Window Title\n'

/**
 * replace for the english version.
 */
function replaceTaskListHeader (taskList) {
  var data = taskList.trim().split("\n")
  data = data.slice(1) // remove first line
  data.unshift(taskListHeader) // insert proper first line
  return data.join("\n") // nothing happened..
}
...
Psaux.prototype.obtainTasklist = function (callback) {
  ...
  var tasklistCmd = 'tasklist.exe /V'
  var wmicCmd = 'wmic process get CommandLine, name, ProcessId'
  var execOpts = { maxBuffer: MAX_BUFFER }

  ChildProcess.exec(tasklistCmd, execOpts, function (err, stdout, stderr) {
    if (err || stderr) return callback(err || stderr.toString())
    var taskList = replaceTaskListHeader(stdout)

    ChildProcess.exec(wmicCmd, execOpts, function (err, stdout, stderr) {
      if (err || stderr) return callback(err || stderr.toString())
      var wmicList = stdout
...
Psaux.prototype.setInterval = function setInterval_ (opts) {
  opts = Object.assign({ parsed: true, interval: 20000 }, opts)
